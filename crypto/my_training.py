...
date = [dt.datetime.strptime(d, '%Y-%m-%d').date() for d in dates]
...
X_train, X_test, Y_train, Y_test = create_Xt_Yt(X, Y, percentage = 1)
...
try:
    ...
    plt.plot(Y_test[:150], color = 'green') # GREEN - actual RESULT
